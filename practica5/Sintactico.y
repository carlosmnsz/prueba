...
bloque : INICIOBLOQUE { TS_InsertaMarca(); } declar_variables_locales declar_subprogs sentencias FINBLOQUE { TS_VaciarEntradas();} ;
...
cuerpo_declar_variables : tipo lista_variables PUNTOYCOMA {listaGlobal=0; tipoGlobal=DESC; fprintf(principal, ";\n");}
	| error ;

lista_variables : variable COMA lista_variables { if(declarVar==1){fprintf(principal, ", %s", $1.lexema);}}
	| variable error lista_variables
	| variable {if(declarVar==1){fprintf(principal, "%s", $1.lexema);}};

variable : IDENT { 
		if(declarVar == 1) { TS_InsertaIdent($1);}
...
asignacion : variable ASIGNACION expresion PUNTOYCOMA
		...
		else {genera(4,$1,$2,$3,$4);}};

si : SI PARIZQ expresion PARDER ENTONCES sentencia SINO sentencia 
	{ if($3.tipo != BOOLEANO) { printf("Error si linea %d: La expresion no es de tipo logico.\n", lineaActual);}}
	| SI PARIZQ expresion PARDER ENTONCES sentencia
	{ if($3.tipo != BOOLEANO) { printf("Error si linea %d: La expresion no es de tipo logico.\n", lineaActual);}} ;
...
expresion : PARIZQ expresion PARDER { $$.tipo = $2.tipo; $$.numDim = $2.numDim;  $$.lexema=$2.lexema;}
	| OPUNARIO expresion {TS_OpUnario($1, $2, &$$);  }
	| SUMRES expresion %prec OPUNARIO {TS_OpSumResUnario($1, $2, &$$); }
	| expresion OPBINARIO expresion {TS_OpMulDiv($1, $2, $3, &$$);  }
	| expresion OPOR expresion {TS_OpOrXor($1, $2, $3, &$$); }
	| expresion OPRELACIONAL expresion {TS_OpRel($1, $2, $3, &$$); }
	| expresion SUMRES expresion { TS_OpSumRes($1, $2, $3, &$$);  }
	| expresion OPAND expresion {TS_OpAnd($1, $2, $3, &$$); }
	| expresion OPBINLISTA expresion {TS_OpBinListas($1, $2, $3, &$$);}
	| expresion ARROBA expresion {TS_OpBinArroba($1, $2, $3, &$$);}
...
tipo : LISTA TIPOBASICO {listaGlobal=1; asignaTipoGlobal($2); if(declarVar){fprintf(principal, "Lista %s", tDatoAString($2.tipo));}}
		| TIPOBASICO {asignaTipoGlobal($1); if(declarVar){fprintf(principal, "%s", tDatoAString($1.tipo));}}
		| error;
