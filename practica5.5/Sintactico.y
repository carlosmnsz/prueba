...
bloque : INICIOBLOQUE { TS_InsertaMarca(); tabulacion++;} declar_variables_locales declar_subprogs sentencias FINBLOQUE { tabulacion--; TS_VaciarEntradas();} ;
...
variable : IDENT { 
		if(declarVar == 1) { TS_InsertaIdent($1); tabular(); generaDecVar($1);}
...
declar_subprog : {tabular();} cabecera_subprograma { subProg = 1; } INICIO sentencias FIN PUNTOYCOMA{ subProg = 0; tabular(); fprintf(principal, "}\n");} ;
...
sentencia : { if(decIf==1){ insertaCond(1); fputs("{\n", principal); decIf++; } } bloque
	...
	| { if(decIf==1){ insertaCond(1); fputs("{\n", principal); decIf++; } } entrada PUNTOYCOMA {fputs("\n", principal);}
	| { if(decIf==1){ insertaCond(1); fputs("{\n", principal); decIf++; } } salida PUNTOYCOMA {fputs("\n", principal);}
...
expresion : PARIZQ expresion PARDER { $$.tipo = $2.tipo; $$.numDim = $2.numDim;  $$.lexema=$2.lexema;}
	...
	| expresion ARROBA expresion MASMAS expresion {TS_OpTerciario($1, $3, $5, &$$); generarTerciario(&$$, $1, $2, $3, $4, $5);}
	| variable { declarVar = 0; $$.lexema=$1.lexema;}
	| constante { $$.tipo = $1.tipo; $$.numDim = $1.numDim; $$.lexema=$1.lexema;}
	| CORIZQ agregado CORDER {$$.tipo=$2.tipo; $$.numDim=1; strcat($$.lexema, $2.lexema); strcat($$.lexema, $3.lexema);}
	| error ;

lista_expresiones : expresion COMA lista_expresiones { numParam++; TS_CompruebaParametro($1, numParam); strcat($$.lexema, ","); strcat($$.lexema, $3.lexema);}
