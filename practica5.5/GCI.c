		...
		if(TS[i].entrada==PAR_FORMAL){
			if(primero==0){
				if(TS[i].numDim>0){
					...
					case ENTERO:
					case BOOLEANO:
						fprintf(principal, "struct listOfInt ");
						break;
					case REAL:
						fprintf(principal, "struct listOfFloat ");
						break;
					case CARACTER:
						fprintf(principal, "struct listOfChar ");
						break;
					default:
						break;
					}
...
void generaDecVar(atributos a){
	...
	if(tipoGlobal == ENTERO){
		if(listaGlobal>0){
			sprintf(sent,"struct listOfInt %s;\n",a.lexema);
		}
		else{
			sprintf(sent,"int %s;\n",a.lexema);
		}
		...
		if(listaGlobal>0){
			sprintf(sent,"struct listOfFloat %s;\n",a.lexema);
		}
		else{
		...
		if(listaGlobal>0){
			sprintf(sent,"struct listOfChar %s;\n",a.lexema);
		}
		else{
			sprintf(sent,"char %s;\n",a.lexema);
		}
